...
@section('content')
    ...
    <form action="{{ route('admin.reservationitems.store') }}" method="POST" enctype="multipart/form-data">
        @csrf
        ...
        <div class="form-group">
            <label for="category_event_id">Task Category</label>
            <select class="form-control" id="category_event_id" name="category_event_id" required>
                @foreach($categoryEvent as $category)
    ...
    <script>
        async function selectInventoryQuantity() {
            let inventory = await fetchInventories();
            let inventoryOptions = inventory.map(inventory => `<option value="${inventory.id}">${inventory.name} (${inventory.quantity} available)</option>`).join('');
